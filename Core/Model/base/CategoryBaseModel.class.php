...
//$CenterProductExtra = Core::ImportExtra( 'CenterProduct' );
					...
					//$totals = $CenterProductModel->GetTotal( $search );
					//$totals = 100 ;
					//return $totals;
					return $this->ModelP->GetTotal( $search );
	}
	...
	function Init()
	{
		...
		$this->ModelP = Core::ImportModel( 'CenterProduct' );
		//$this->ModelQ = Core::ImportBaseClass( 'Model' );
	...
	function GetAllCategory()
	{
		...
		$this->categoryList = $this->NC->GetCategory();
...
//$search = array();
//$search['category_id'] = intval($pid);
//$total = $CenterProductModel->GetTotal( array( 'category_id' => intval($pid) ) );
//echo $pid ;
...
//$total = 100 ;
				...
				if ( $val['pid'] == $pid )
				{
					...
					//if($val['pid']>0)
					//{
					$p_total = $this -> ProductTotal( $val['id'] );
					//}
					//else{$p_total=999;}
					
					//$data = array();
					//$data['product_total'] = $p_total;
					//$this->Update( $val['id'], $data );
					
					
					//$cache[$count]['text'] = $val['name'] . "(ID:{$val['id']})" . "(" . $p_total . ")";
					if($p_total>0){$cache[$count]['text'] ="(ID:{$val['id']})".$val['name'] . "(" . $p_total . ")";}
					else{$cache[$count]['text'] ="(ID:{$val['id']})". $val['name'];}
	...
	function GetTwoLevelList( $noHidden = 0 )
	{
		$list = $this->GetTopList( $noHidden );

		if ( !$list )
			return $list;

		$childList = $this->GetListByParentId( array_keys( $list ), $noHidden );
		$childList = ArrayGroup( $childList, 'pid', true );

		foreach ( $list as $key => $val )
		{
			$list[$key]['child_list'] = $childList[$val['id']];
		}

		return $list;
	}

	function GetListByParentId( $ids, $noHidden = 0 )
	{
		if ( is_array( $ids ) )
			$ids = implode( ',', $ids );
	...
	function GetTopList( $noHidden = 0 )
	{
		$condition = array();
		$condition['pid'] = '0';

		if ( $noHidden )
			$cfg['condition']['hidden'] = 0;
	...
	function Del( $id )
	{
		return $this->Model->Del( array( 'table' => $this->table, 'condition' => array( 'id' => $id ) ) );
	}
