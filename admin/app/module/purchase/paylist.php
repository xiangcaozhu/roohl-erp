...
//$search['status']= (int)$_GET['status'];
$onePage=20;
if((int)$_GET['status']==1){$onePage=999;}
list( $page, $offset, $onePage ) = Common::PageArg( $onePage );


$total = $CenterPurchaseModel->GetPayListTotal( $search );

$list = $CenterPurchaseModel->GetPayList(  $search, $offset, $onePage );

/*
foreach ( $list as $key => $val )
{
	$purchase_all = explode(",",$val['purchase_id']);
	
	$purchase_list = array();
	foreach ( $purchase_all as $key_a )
	{
	//echo $key_a.'<br>';
	$purchase_info = array();
	$purchase_info = $CenterPurchaseModel->Get($key_a);
	$Admins = $AdminArray -> GetAdministrator( $purchase_info['user_id'] );
	$purchase_info['user_name_zh'] = $Admins['user_real_name'] ;
	$purchase_info['payment_type_name'] = $purchasePaymentTypeList[$purchase_info['payment_type']];


$purchaseProductList = $CenterPurchaseModel->GetProductList( $key_a );
$purchaseProductList = $CenterPurchaseExtra->ExplainProduct_1( $purchaseProductList );
$purchase_info['productList'] = $purchaseProductList;
	
	
	$purchase_list[] = $purchase_info;

	}
	
	$list[$key]['purchase_list']=$purchase_list;
}
*/
...
$CenterSupplierModel = Core::ImportModel( 'CenterSupplier' );
$tpl['Supplier_list']  = $CenterSupplierModel->GetList();



$tpl['list']  = $list;
$tpl['user_group']  = $__UserAuth['user_group'];
...
Common::PageOut( 'purchase/paylist.html', $tpl );
