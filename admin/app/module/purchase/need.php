...
$needList = $CenterOrderModel->GetNeedList_one1009($val_a['supplierId'] );
...
/*
	$tmp = $CenterOrderModel->GetSupplierNeedProductList( $val['sku_id'],$val_a['supplierId'] );

	foreach ( $tmp as $k => $v )
	{
		$tmp[$k]['add_time'] = DateFormat( $v['add_time'] );
		$tmp[$k]['channelName'] = $CenterOrderModel->GetChannelName( $v['channelID'] );
		
		
		//$tmp[$k]['purchaseCheck'] = trim(str_replace('有货','',$ActionLogModel->GetLast( $v['order_id'], 'order_check_purchase' )));
		//$tmp[$k]['serviceCheck'] = trim($ActionLogModel->GetLast( $v['order_id'], 'order_check_service' ));
