...
if ( $__UserAuth['user_group'] == 13 && $val['sign_pro_mg'] < 0 )
	$list[$key]['sign_pro_mg'] = ' <font>→ [ 产品经理审核：</font><a href="?mod=purchase.check&id='.$val['id'].'&checking=yes&go=1&payup=1" onclick="return confirm(\'确定要审核通过吗?\');">通过</a> / <a style="" onclick="ActionLog('.$val['id'].', 1, 1);">拒绝</a> <font>]</font>';
...
if ( $__UserAuth['user_group'] == 14 && $val['sign_ope_mj'] < 0 && $val['sign_pro_mg']>0 )
	$list[$key]['sign_ope_mj'] = ' <font>→ [ 产品总监审核：</font><a href="?mod=purchase.check&id='.$val['id'].'&checking=yes&go=1&payup=1" onclick="return confirm(\'确定要审核通过吗?\');">通过</a> / <a style="" onclick="ActionLog('.$val['id'].', 1, 1);">拒绝</a> <font>]</font>';
