...
list( $page, $offset, $onePage ) = Common::PageArg( 3 );
global $__UserAuth;
global $__Config;
...
switch ( $__UserAuth['user_group'] )
		{
			case 12:
			    $search = array( 'status' => 2, 'user_id' =>  $__UserAuth['user_id'] );
			break;
			case 13:
			    $search = array( 'status' => 2, 'sign_top_jl' =>  $__UserAuth['user_id'] );
			break;
			case 14:
			...
			break;
			default:
			    $search = array( 'status' => 2, 'user_id' =>  -1 );
			break;
}
...
if($val['user_id'] == $__UserAuth['user_id'])
  {
    $list[$key]['del_bottom'] = '<a style="display:none" href="?mod=purchase.del&id='.$val['id'].'&go=1" onclick="return confirm(\'确定删除吗?\');">删除</a> / <a style="display:none" href="?mod=purchase.edit&id='.$val['id'].'&checking=yes&go=1">修改</a>';
    $list[$key]['print_bottom'] = '<td width="80" align="center"><a href="?mod=purchase.print&id='.$val['id'].'" target="_blank">打印采购单</a></td>';
	
	}

if( ($val['user_id'] == $__UserAuth['user_id'] && $val['pay_status'] > 0) || ($val['payment_type'] ==1 && $val['user_id'] == $__UserAuth['user_id'] && $val['pay_status'] < 0) )
    $list[$key]['del_bottom'] = '<a href="?mod=purchase.new.Donext&id='.$val['id'].'" onclick="return confirm(\'所有执行操作已完成，确认提交到下个工作流吗?\');">确认完成</a>';

if($val['user_id'] == $__UserAuth['user_id'] && $val['pay_status'] < 0 && $val['payment_type'] ==2 )
    $list[$key]['del_bottom'] = "";
...
if ( $__UserAuth['user_group'] == 13 && $val['sign_pro_mg'] < 0 )
	$list[$key]['sign_pro_mg'] = ' <font>→ [ 产品经理审核：</font><a href="?mod=purchase.check&id='.$val['id'].'&checking=yes&go=1&payup=1" onclick="return confirm(\'确定要审核通过吗?\');">通过</a> / <a  style="display:none" style="" onclick="ActionLog('.$val['id'].', 1, 1);">拒绝</a> <font>]</font>';
...
if ( $__UserAuth['user_group'] == 14 && $val['sign_ope_mj'] < 0 && $val['sign_pro_mg']>0 )
	$list[$key]['sign_ope_mj'] = ' <font>→ [ 产品总监审核：</font><a href="?mod=purchase.check&id='.$val['id'].'&checking=yes&go=1&payup=1" onclick="return confirm(\'确定要审核通过吗?\');">通过</a> / <a style="display:none" onclick="ActionLog('.$val['id'].', 1, 1);">拒绝</a> <font>]</font>';
...
if ( $__UserAuth['user_group'] == 15 && $val['sign_ope_vc'] < 0 && $val['sign_ope_mj'] >=0 )
     $list[$key]['sign_ope_vc'] = ' <font>→ [ 产品副总审核：</font><a href="?mod=purchase.check&id='.$val['id'].'&checking=yes&go=1" onclick="return confirm(\'确定要审核通过吗?\');">通过</a> / <a style="display:none" onclick="ActionLog('.$val['id'].', 1, 1);">拒绝</a> <font>]</font>';
...
if ( $val['pay_status'] ==-1 )
	$list[$key]['pay_lock_user'] = ' <font>→ [ 财务审核中：</font><strong>'.$val['pay_lock_user_name'].'</strong><b>≌</b> <font>]</font>';
...
if ( $__UserAuth['user_group'] == 16 && $val['pay_status'] == -1 )
     $list[$key]['pay_user'] = ' <font>→ [ </font><a href="?mod=purchase.new.payup&id='.$val['id'].'&pay_status=1" onclick="return confirm(\'确定付款已经完成吗?\');">付款完成</a> / <a style="" onclick="ActionLog('.$val['id'].', 1, 1);">付款错误</a> <font>]</font>';
...
if( $val['user_id'] == $__UserAuth['user_id'] )
	$list[$key]['pay_user'] = ' <font>→ [ 采购单修改中</font>  / <a style="display:none" href="?mod=purchase.new.recall&id='.$val['id'].'" onclick="return confirm(\'确定重新提报吗?\');">重新提报</a> <font>]</font>';
else
...
Common::PageOut( 'purchase/listchecking.html', $tpl );
}
?>