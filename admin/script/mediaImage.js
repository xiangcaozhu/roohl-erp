var __GlobalImageCategoryPath = [];
var __GlobalImageCategoryId = 0;
var __GlobalImagePage = 1;

function MediaImageBox(siteId, siteName, callHandler){
	var html = '';
	...
	Dialog( '图片库 当前站点:'+siteName, html, CloseBox, CloseBox, OpenTree, '完成', '关闭', 200 );

	var swfUploaderConfig = {
		upload_url: "?mod=cms.media.image.upload",
		file_post_name: "file",
	...
	function Load(page){
		...
		__GlobalImagePage = page;

		$('#image_box_list').empty();
		$('#image_box_list').html('<div><img src="image/rule-ajax-loader.gif" align="absmiddle">加载中...</div>');
		$.ajax({
			url:'?mod=cms.media.image.json',
			data:{cid:categoryId, page:page, site:siteId, rand:Math.random()},
			dataType: 'json',
	...
	function OpenTree(){
		...
		tree = new Ext.tree.TreePanel({
			...
			loader: new Ext.tree.TreeLoader({
				dataUrl: '?mod=cms.media.image.category.json&site='+siteId,
		...
		tree.on('expandnode',function(node){
			if (node.childNodes.length>0){
				var pathList = __GlobalImageCategoryPath;
				var path = node.getPath();

				if ($.inArray(path,pathList)==-1){
					pathList[pathList.length] = node.getPath();
					__GlobalImageCategoryPath = pathList;
				}
			}
		});
		...
		tree.getLoader().on('load',function(){
			...
			if (__GlobalImageCategoryId){
				var selectNode = tree.getNodeById(__GlobalImageCategoryId);
				if (selectNode){
					selectNode.select();
					categoryId =__GlobalImageCategoryId;
					Load(__GlobalImagePage);
				}
			}
		});
		...
		tree.on('click',function(node){
			__GlobalImageCategoryId = node.id;
			__GlobalImagePage = 1;
			categoryId = node.id;
			Load(1);
		});
