...
window.calendar = null;/**< global object that remembers the calendar */ // initialize the preferences object; // embed it in a try/catch so we don't have any surprises try { Zapatec.Calendar.loadPrefs(); } catch(e) {};
...
Zapatec.Calendar.i18n=function(str,type){
	...
	if(!type){
		...
		case"dn":tr=Zapatec.Calendar._TT._DN[str];
		break;
		case"sdn":tr=Zapatec.Calendar._TT._SDN[str];
		break;
		case"mn":tr=Zapatec.Calendar._TT._MN[str];
		break;
		case"smn":tr=Zapatec.Calendar._TT._SMN[str];
		break;
		case"ampm":tr=Zapatec.Calendar._TT._AMPM[str];
		break;
	}
	if(!tr)tr=""+str;
	return tr;
...
Zapatec.Calendar.prefs={
	fdow:null,history:"",sortOrder:"asc",hsize:9
};
...
Zapatec.Calendar.cellClick=function(el,ev){
	...
	if(typeof el.navtype=="undefined"){
		...
		switch(el.navtype){
			case 400:Zapatec.Utils.removeClass(el,"hilite");
			...
			return;
			case 50:var date=cal.currentDate;
			...
			cal.onUpdateTime();
			return;
			case 201:case 202:var cel=el.timePart;
			var date=cal.currentDate;
			...
			cal.onUpdateTime();
			return;
			case 0:if(cal.getDateStatus&&((cal.getDateStatus(date,date.getFullYear(),date.getMonth(),date.getDate())==true)||(cal.getDateStatus(date,date.getFullYear(),date.getMonth(),date.getDate())=="disabled"))){
				return false;
			}
			break;
		}
	...
	if(closing){
		//Zapatec.Utils.removeClass(el,"hilite");cal.callCloseHandler();
	}
	};
...
Zapatec.Calendar.prototype.create=function(_par){
	...
	for(var l=1;l<=rowsOfMonths;++l){
		...
		if(this.numberMonths!=1){
			...
			for(var k=1;(k<=this.monthsInRow)&&((l-1)*this.monthsInRow+k<=this.numberMonths);
			++k){
				hd(row,1);
		...
		for(k=1;(k<=this.monthsInRow)&&((l-1)*this.monthsInRow+k<=this.numberMonths);
		++k){
			if(this.weekNumbers){
				cell=Zapatec.Utils.createElement("td",row);
...
Zapatec.Calendar.prototype._init=function(firstDayOfWeek,date,last){
		...
		var loadday=$("#begin_date").val().replace("-",",");
	    var mytoday=new Date(loadday),TD=today.getDate(),TY=today.getFullYear(),TM=today.getMonth();
		date = mytoday;
	...
	for(var l=1;l<=rowsOfMonths;++l){
		...
		for(var i=7;--i>0;row=row.nextSibling){
			...
			++k){
				...
				var iday;
				for(j=7;cell&&(iday=date.getDate())&&(j>0);
				date.setDate(iday+1),((date.getDate()==iday)?(date.setHours(1)&&date.setDate(iday+1)):(false)),cell=cell.nextSibling,--j){
					var
...
Zapatec.Calendar.prototype._displayWeekdays=function(){
	...
	var weekend=Zapatec.Calendar.i18n("WEEKEND");
	for(k=1;(k<=this.monthsInRow)&&(cell);
	++k){
		for(var i=0;i<7;++i){
...
Date.parseDate=function(str,format){
	...
	while(fmt){
		...
		switch(token){
			...
			case"%j":valueLength=3;
			break;
			case"%u":case"%w":valueLength=1;
			case"%y":case"%m":case"%d":case"%W":case"%H":case"%I":case"%M":case"%S":{
				break;
			}
		}
...
Zapatec.Calendar.setup=function(params){
	...
	function onSelect(cal){
		...
		//location.href="?mod=report.dataone&datanow="+params.inputField.value+""
		location.href="?mod=report.dataone&datanow="+params.inputField.value
