...
* Created on : 2017-11-18 00:21:48
*/
?>
...
<span style="float:left;">采购单号：<b><?php
if ( $U_ID == 68 )
{
...
{
?><font color="#0066CC"><?php echo $val['id']; ?></font><?php
}
?></b>　　下单日期：<b><font color="#0066CC"><?php echo $val['add_time']; ?></font></b><?php
if ( $U_ID == 68 )
{
?><a target="_blank" href="?mod=purchase.E_CGD&id=<?php echo $val['id']; ?>&del=1">删除</a><?php
}
?></span>
...
?>
								<tr>
									...
									<td align="center" width="70" height="22" style="padding-top:3px;"><?php echo FormatMoney($rows['sale_price']); ?></td>
								</tr>
								<?php
}
}
...
<td align="center" bgcolor="#FFFFFF"><?php echo $row['quantity']; ?></td>
<td align="center" bgcolor="#FFFFFF">&yen;<?php echo $row['price']; ?></td>
<?php
if ( $val['type'] == 2 )
{
?><td align="center" bgcolor="#FFFFFF">&yen;<?php echo $row['help_cost']; ?></td><?php
}
?>
...
?>" align="right" bgcolor="#FFFFFF" style="padding:5px;"><b>合计：￥<?php echo $val['all_money']; ?></b></td>
		</tr>
	</tbody>
</table>
</div>
...
{
?>
<div style="float:left;width:740px; padding-bottom:10px;">
<?php
if ( $val['orderList'] )
{
...
{
?>
<div style="float:left; padding:5px 10px;width:140px;border:#999999 1px dashed; background:#F1F1F1;margin:5px 10px;">
订单：<a href="?mod=order.list&id=<?php echo $rowp['id']; ?>" target="_blank"><?php echo $rowp['id']; ?></a> → <?php
...
<div style="float:left; padding:5px 10px;width:140px;border:#999999 1px dashed; background:#F1F1F1;line-height:1.7;">
制单人员：<b><font color="#0066CC"><?php echo $val['user_name_zh']; ?><?php echo $val['user_me']; ?></font></b><br />
付款方式：<b><font color="#0066CC"><?php echo $val['payment_type_name']; ?></font></b><br />
采购方式：<b><?php echo $val['type_name']; ?></b>
</div>
<div style="float:left; padding:5px 10px;width:140px;border:#999999 1px dashed; background:#F1F1F1;margin-top:10px;">
入库状态：<b><?php echo $val['into_status_name']; ?></b>
</div>
...
　<a href="?mod=purchase.printZC&id=<?php echo $val['id']; ?>" target="_blank">支出单</a>
</div>
...
?>
<div style="float:left; padding:5px 10px;width:140px;border:#999999 1px dashed; background:#F1F1F1;margin-top:10px;">
　<a href="?mod=purchase.printWL&id=<?php echo $val['id']; ?>" target="_blank">发货单</a>
　<a href="?mod=purchase.printWL_excel&id=<?php echo $val['id']; ?>&name=<?php echo $val['supplier_name']; ?>" target="_blank">导出发货单</a>
</div>
<?php
}
?>
...
<ul style=" position:fixed;top:270px;left:270px;width:250px;margin:0px; padding:0px;" class="xiao_1">
...
采购单号：<input style="width:60px;" type="text" name="id" id="id" value="<?php echo $_GET['id']; ?>">
</div>
<div style="float:left;width:228px;height:25px;">
...
<select name="suppSSlier_id" style="width:225px">
...
<select name="into_status" style="width:93px">
<option value="">入库状态</option>
<option value="1"  <?php
if ( $_GET['into_status'] == 1 && $_GET['into_status'] != '' )
{
?>selected<?php
}
?> >　未入库</option>
<option value="2"  <?php
if ( $_GET['into_status'] == 2 && $_GET['into_status'] != '' )
{
?>selected<?php
}
?> >　部分入库</option>
<option value="3"  <?php
if ( $_GET['into_status'] == 3 && $_GET['into_status'] != '' )
{
?>selected<?php
}
?> >　全部入库</option>
