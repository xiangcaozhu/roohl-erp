...
* Created on : 2013-01-08 09:57:28
*/
?>
...
<div class="HY-content-header clearfix-overflow">
	<h3><?php echo $warehouse_info['name']; ?> 其他收货</h3>
	<div class="right">
...
function SubmitForm(){
	...
	Loading();
	$.ajax({
		url: '?mod=receive.new.other<?php echo $warehouse_uri; ?>&rand=' + Math.random(),
		type:'POST',
		data:post,
		success: function(info){
			if (info=='200'){
				Loading('处理成功', '正在跳转到列表页面...');
				window.location='?mod=receive.list<?php echo $warehouse_uri; ?>';
			}else{
				alert(info);
				UnLoading();
			}
